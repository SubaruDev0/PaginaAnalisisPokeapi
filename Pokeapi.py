...
def obtener_info_pokemon(pokemones):
    info_pokemon = []  # Lista para almacenar los datos de los Pokémon

    # Iterar sobre la lista de Pokémon seleccionados
    for nombre in pokemones:
        try:
            # Realizar una solicitud para obtener los datos del Pokémon
            ...
            # Obtener el número de Pokedex (índice único de cada Pokémon)
            numero_pokedex = data['id']

            # Obtener la URL de la especie del Pokémon para poder saber su generación
            ...
            # Extraer la generación del Pokémon desde la información de la especie
            generacion = especies_data['generation']['name']

            # Extraer los tipos del Pokémon (puede tener más de uno)
            tipos = [tipo['type']['name'] for tipo in data['types']]

            # Extraer las estadísticas (HP, ataque, defensa, etc.)
            info_pokemon.append({
                'Número de Pokedex': numero_pokedex,  # Número único del Pokémon
                'Nombre': nombre,  # Nombre del Pokémon
                ...
                'Tipos': tipos  # Tipos de Pokémon (agua, fuego, etc.)
            })
    ...
    return info_pokemon  # Devolver la lista con la información de los Pokémon
...
def main():
    ...
    # Layout con dos columnas: una para el multiselect y otra para el botón
    col1, col2 = st.columns([3, 1])

    # En la primera columna: multiselect para seleccionar Pokémon
    with col1:
        input_pokemon = st.multiselect("Selecciona los Pokémon para ver su información:", lista_pokemon)

    # En la segunda columna: botón para seleccionar Pokémon aleatorios
    with col2:
        if st.button("Random"):
            input_pokemon = random.sample(lista_pokemon, 10)  # Seleccionar 10 Pokémon aleatorios
    ...
    if input_pokemon:
        ...
        if info:
            ...
            # (Los gráficos y demás funciones permanecen iguales que en tu código original)
