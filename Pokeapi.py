...
def obtener_pokemon():
    ...
    def obtener_info_pokemon(pokemones):
        ...
        for nombre in pokemones:
            try:
                ...
                info_pokemon.append({
                    'Número de Pokedex': numero_pokedex,
                    'Nombre': nombre,
    ...
    def main():
        ...
        # Botón para elegir 10 Pokémon aleatorios
        if st.button("Random"):
            random_pokemon = random.sample(lista_pokemon, 10)
            input_pokemon.extend(random_pokemon)
            input_pokemon = list(set(input_pokemon))  # Eliminar duplicados
            st.experimental_rerun()  # Refrescar la app para que los seleccionados se actualicen

        # Procesar los Pokémon seleccionados
        if input_pokemon:
